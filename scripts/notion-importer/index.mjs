...
const ASTRO_ASSETS_PATH = join(__dirname, '..', '..', 'src', 'assets', 'image');
...
function copyToAstroContent(outputDir) {
    ...
    try {
        ...
        if (existsSync(ASTRO_CONTENT_PATH)) {
            ...
            // Remove the subdirectory from image paths since we copy images directly to src/assets/image/
            updatedContent = updatedContent
                .replace(/\.\/assets\/image\/[^\/]+\//g, '../assets/image/')
                .replace(/\.\.\/assets\/image\/[^\/]+\//g, '../assets/image/');
